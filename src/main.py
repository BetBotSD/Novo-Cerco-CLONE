...
try:
    ...
    from src.cerco import mobile_cerco
...
def run_cycle(orchestrator: CercoOrchestrator) -> None:
    ...
    - Envia oportunidades de arbitragem ao Mobile (PostgreSQL + Push).
    """
    ...
    for match in match_results:
        ...
        # Canal 1: Telegram (mantido 100% intacto)
        ...
        # Canal 2: Mobile (novo)
        mobile_cerco.publish_match_result(
            match,
            stake_base=stake_base,
            with_image=with_image,
        )
    ...
    logger.info("[CERCO] Mensagens enviadas ao Telegram neste ciclo: %d", sent_telegram)
    logger.info("[CERCO] Oportunidades publicadas no Mobile neste ciclo: %d", sent_mobile)
